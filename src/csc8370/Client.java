...
public class Client {
    ...
    public void callServer(String hostName,int portNumber) throws ClassNotFoundException, UnknownHostException, IOException {
        ...
        chat = new ChatCommunication(echoSocket, in, out,"Client"); 
        Packet pkt = new Packet(new Message("Hey! Want to chat? My public keys are: " + chat.myPublicKey+" and "+chat.myModulus));
        System.out.println("Hey! want to chat?");
        //System.out.println("Me : "+pkt.core.text);
        System.out.println("<sent at: "+pkt.timestamp+">\n");
        out.writeObject(pkt);
        wait.start();
        Packet reply = (Packet)(in.readObject());
